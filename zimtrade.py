...
class ZimTrade(QMainWindow):
    ...
    def automated_trading(self):
        ...
        # Save data to file and database
        self.save_market_data(rates)
    ...
    def save_market_data(self, rates):
        # Save market data to a CSV file
        rates.to_csv("market_data.csv", mode='a', header=not pd.io.common.file_exists("market_data.csv"))
        ...
        for index, row in rates.iterrows():
            try:
                c.execute('''
                    INSERT INTO market_data (time, open, high, low, close) VALUES (?, ?, ?, ?, ?)
                ''', (index, row['open'], row['high'], row['low'], row['close']))
            except sqlite3.IntegrityError:
                print(f"Skipping duplicate entry for {index}")
                continue
